...
def visualize_rays(camera, resolution=10, ray_length=1000):
...
def cull_hidden_faces_by_camera(obj, camera, resolution=100):
    ...
    for x in range(resolution):
        for y in range(resolution):
            ...
            # Convert start and end points to world space
            start_world = camera.matrix_world @ start
            end_world = camera.matrix_world @ start - (camera.matrix_world @ end - camera.matrix_world @ start).normalized()
